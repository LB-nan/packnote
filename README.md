...
### 11、打包输出的路径优化
1、可以在输出的文件名前面加上文件夹名字，打包出来就会自动新建文件夹。

	 output: { 
		//                             这里在dist目录下新建了一个assets的文件
        path: path.resolve(__dirname, 'dist/assets'),
		// 这里会在里面再次建一个js文件夹存放`main.js`
        filename: 'js/main.js'
    },
    plugins: [
...
### 12、publicPath:所有资源的基础路径，以`/`结尾
1、当设置了打包的路径打包的时候会发生路径错误找不到文件的情况，这个时候就需要设置`publicPath`了。`publicPath`设置的值是一个字符串，他会在所有的路径前面加上这个字符串来，这个字符串是路径外面的文件夹，我上面设置的文件名字叫`assets`所以我们这里也设置成为那个。
...
5、可以用`env`来代替`es2015`等预设，安装方法一样，配置方法老地方，`env`只能搞定已经列入标准的语法，比如`...`扩展运算符还没有列入标准就需要用另一个来搞定`transform-object-rest-spread`，安装方法一样。
